"""
Load this file to your GDB session to enable pretty-printing of some Bradot C++ types.

GDB command: `source misc/utility/bradot_gdb_pretty_print.py`.
...
...
    {
        "description": "Load custom pretty-printers for Bradot types.",
        "text": "source ${workspaceFolder}/misc/utility/bradot_gdb_pretty_print.py"
    }
]
```
...
# Printer for Bradot StringName variables.
class BradotStringNamePrinter:
...
# Printer for Bradot String variables.
class BradotStringPrinter:
    def __init__(self, value):
        self.value = value
...
# Printer for Bradot Vector variables.
class BradotVectorPrinter:
    def __init__(self, value):
        self.value = value
...
def lookup_pretty_printer(value):
    if value.type.name == "StringName":
        return BradotStringNamePrinter(value)
    if value.type.name == "String":
        return BradotStringPrinter(value)
    if value.type.name and VECTOR_REGEX.match(value.type.name):
        return BradotVectorPrinter(value)
    return None
