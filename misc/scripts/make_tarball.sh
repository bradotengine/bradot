...
if [ ! -e "version.py" ]; then
  echo "This script should be ran from the root folder of the Bradot repository."
  exit 1
fi
...
while getopts "h?sv:g:" opt; do
  ...
  h|\?)
    ...
    echo
    echo "  -s script friendly file name (bradot.tar.gz)"
...
if [ ! -z "$script_friendly_name" ]; then
  ...
  if [ ! -z "$bradot_version" ]; then
    NAME=godot-$bradot_version
  else
    NAME=godot-$HEAD
  fi
fi
