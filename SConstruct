...
opts.Add(BoolVariable("production", "Set defaults to build Bradot for use in production", False))
...
opts.Add("vsproj_name", "Name of the Visual Studio solution", "bradot")
...
if cc_version_major == -1:
    ...
    if cc_version_major < 9:
        print_error(
            ...
            "C++17, or has bugs when compiling Bradot. Supported versions are 9 "
    ...
    if cc_version_major == 16 and cc_version_minor < 11:
        print_error(
            ...
            "when compiling Bradot. Use a newer VS2019 version, or VS2022."
        )
        Exit(255)
    if cc_version_major == 15 and cc_version_minor < 9:
        print_error(
            "Detected Visual Studio 2017 version older than 15.9, which has bugs "
            "when compiling Bradot. Use a newer VS2017 version, or VS2019/VS2022."
        )
        Exit(255)
    if cc_version_major < 15:
        print_error(
            "Detected Visual Studio 2015 or earlier, which is unsupported in Bradot. "
...
# Disable exception handling. Bradot doesn't use exceptions anywhere, and this
