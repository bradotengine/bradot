...
Apply `patches` to add the new options required by Bradot and general compilation fixes.
...
Important: Some files have Bradot-made changes for use with MinGW.
They are marked with `/* BRADOT start */` and `/* BRADOT end */`
comments.
...
- All `.cpp` files listed in `modules/raycast/bradot_update_embree.py`
- All header files in the directories listed in `modules/raycast/bradot_update_embree.py`
- All config files listed in `modules/raycast/bradot_update_embree.py`
- `LICENSE.txt`

The `modules/raycast/bradot_update_embree.py` script can be used to pull the
...
to be usable by Bradot's socket implementation and allow IPv6 and DTLS.
Apply the patches in the `patches/` folder when syncing on newer upstream
commits.

Three files (`bradot.cpp`, `enet/bradot.h`, `enet/bradot_ext.h`) have been added to
provide ENet socket implementation using Bradot classes.
...
- The `icudt76l.dat` built with the provided `bradot_data.json` config file (see
...
3. Reconfigure ICU with custom data config:
   `ICU_DATA_FILTER_FILE={BRADOT_SOURCE}/thirdparty/icu4c/bradot_data.json ./runConfigureICU {PLATFORM} --with-data-packaging=common`
4. Delete `data/out` folder and rebuild data: `cd data && rm -rf ./out && make`
5. Copy `source/data/out/icudt76l.dat` to the `{BRADOT_SOURCE}/thirdparty/icu4c/icudt76l.dat`
...
Some Bradot-specific changes are applied via patches included in the `patches` folder.
...
Patch `bradot-node-debug-fix.patch` workarounds shadowing of Bradot's Node class
...
- Added 2 files `bradot_core_mbedtls_platform.c` and `bradot_core_mbedtls_config.h`
  providing configuration for light bundling with core
- Added the file `bradot_module_mbedtls_config.h` to customize the build
...
Once copied, apply `bradot.patch` (needed because Bradot is built without exceptions
...
The only modified file is `src/miniupnpcstrings.h`, which was created for Bradot
...
Important: Some files have Bradot-made changes for use in core/io.
They are marked with `/* BRADOT start */` and `/* BRADOT end */`
...
Collection of single-file libraries used in Bradot components.
...
- `clipper.{cpp,hpp}`
  ...
  * Version: 6.4.2 (2017) + Bradot changes (added optional exceptions handling)
  * License: BSL-1.0
- `cubemap_coeffs.h`
...
- `polypartition.{cpp,h}`
  ...
  * Modifications: Change from STL to Bradot types (see provided patch).
  * License: MIT
- `qoa.h`
...
Important: Nearly all files have Bradot-made changes and renames
...
proposed by these libraries and better integrate them with Bradot.
...
`// -- BRADOT begin --` and `// -- BRADOT end --` comments.
...
Some downstream changes have been made and are identified by
`// -- BRADOT start --` and `// -- BRADOT end --` comments.
...
`vk_mem_alloc.cpp` is a Bradot file and should be preserved on updates.
...
- Run `cmake .` to generate `config.h` and `wslayver.h`
  Contents might need tweaking for Bradot, review diff
- All `.c` and `.h` files from `lib/`
- All `.h` in `lib/includes/wslay/` as `wslay/`
- `wslay/wslay.h` has a small Bradot addition to fix MSVC build
