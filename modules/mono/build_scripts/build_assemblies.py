...
def build_bradot_api(msbuild_tool, module_dir, output_dir, push_nupkgs_local, precision, no_deprecated):
    target_filenames = [
        "BradotSharp.dll",
        "BradotSharp.pdb",
        "BradotSharp.xml",
        "BradotSharpEditor.dll",
        "BradotSharpEditor.pdb",
        "BradotSharpEditor.xml",
        "BradotPlugins.dll",
        "BradotPlugins.pdb",
        "BradotPlugins.runtimeconfig.json",
    ]

    for build_config in ["Debug", "Release"]:
        editor_api_dir = os.path.join(output_dir, "BradotSharp", "Api", build_config)
        ...
        if precision == "double":
            args += ["/p:BradotFloat64=true"]
        if no_deprecated:
            args += ["/p:BradotNoDeprecated=true"]

        sln = os.path.join(module_dir, "glue/BradotSharp/BradotSharp.sln")
        ...
        core_src_dir = os.path.abspath(os.path.join(sln, os.pardir, "BradotSharp", "bin", build_config))
        editor_src_dir = os.path.abspath(os.path.join(sln, os.pardir, "BradotSharpEditor", "bin", build_config))
        plugins_src_dir = os.path.abspath(os.path.join(sln, os.pardir, "BradotPlugins", "bin", build_config, "net6.0"))
...
def generate_sdk_package_versions():
    ...
    version_defines = (
        [
            f"BRADOT{version.major}",
            f"BRADOT{version.major}_{version.minor}",
            f"BRADOT{version.major}_{version.minor}_{version.patch}",
        ]
        + [f"BRADOT{v}_OR_GREATER" for v in range(4, version.major + 1)]
        + [f"BRADOT{version.major}_{v}_OR_GREATER" for v in range(0, version.minor + 1)]
        + [f"BRADOT{version.major}_{version.minor}_{v}_OR_GREATER" for v in range(0, version.patch + 1)]
    )

    props = """<Project>
  <PropertyGroup>
    <PackageVersion_BradotSharp>{0}</PackageVersion_BradotSharp>
    <PackageVersion_Bradot_NET_Sdk>{0}</PackageVersion_Bradot_NET_Sdk>
    <PackageVersion_Bradot_SourceGenerators>{0}</PackageVersion_Bradot_SourceGenerators>
    <BradotVersionConstants>{1}</BradotVersionConstants>
  </PropertyGroup>
</Project>
""".format(version_str, ";".join(version_defines))
    ...
    constants = """namespace Bradot.SourceGenerators
{{
// TODO: This is currently disabled because of https://github.com/dotnet/roslyn/issues/52904
#pragma warning disable IDE0040 // Add accessibility modifiers.
    partial class Common
    {{
        public const string VersionDocsUrl = "https://docs.bradotengine.org/en/{docs_branch}";
    }}
}}
""".format(**version_info)
...
def build_all(
    msbuild_tool, module_dir, output_dir, bradot_platform, dev_debug, push_nupkgs_local, precision, no_deprecated
):
    # Generate SdkPackageVersions.props and VersionDocsUrl constant
    generate_sdk_package_versions()

    # Bradot API
    exit_code = build_bradot_api(msbuild_tool, module_dir, output_dir, push_nupkgs_local, precision, no_deprecated)
    if exit_code != 0:
        return exit_code

    # BradotTools
    sln = os.path.join(module_dir, "editor/BradotTools/BradotTools.sln")
    args = ["/restore", "/t:Build", "/p:Configuration=" + ("Debug" if dev_debug else "Release")] + (
        ["/p:BradotPlatform=" + bradot_platform] if bradot_platform else []
    )
    if push_nupkgs_local:
        args += ["/p:ClearNuGetLocalCache=true", "/p:PushNuGetToLocalSource=" + push_nupkgs_local]
    if precision == "double":
    ...
    if no_deprecated:
        args += ["/p:BradotNoDeprecated=true"]
    sln = os.path.join(module_dir, "editor/Bradot.NET.Sdk/Bradot.NET.Sdk.sln")
    exit_code = run_msbuild(msbuild_tool, sln=sln, chdir_to=module_dir, msbuild_args=args)
    if exit_code != 0:
        return exit_code
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Builds all Bradot .NET solutions")
    parser.add_argument("--godot-output-dir", type=str, required=True)
    parser.add_argument(
        "--dev-debug",
        action="store_true",
        default=False,
        help="Build BradotTools and Bradot.NET.Sdk with 'Configuration=Debug'",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Build BradotSharp without using deprecated features. This is required, if the engine was built with 'deprecated=no'.",
    )
    ...
    output_dir = os.path.abspath(args.bradot_output_dir)
