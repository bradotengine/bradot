// This file contains methods that existed in a previous version of Bradot in ClassDB but were removed
...
{
    /// <inheritdoc cref="SegmentIntersectsConvex(Vector3, Vector3, Bradot.Collections.Array{Plane})"/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public static Vector3[] SegmentIntersectsConvex(Vector3 from, Vector3 to, Bradot.Collections.Array planes)
    {
        return SegmentIntersectsConvex(from, to, new Bradot.Collections.Array<Plane>(planes));
    }
}
...
{
    /// <inheritdoc cref="AddSurface(Mesh.PrimitiveType, Bradot.Collections.Array, Bradot.Collections.Array{Bradot.Collections.Array}, Bradot.Collections.Dictionary, Material, string, ulong)"/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public void AddSurface(Mesh.PrimitiveType primitive, Bradot.Collections.Array arrays, Bradot.Collections.Array<Bradot.Collections.Array> blendShapes, Bradot.Collections.Dictionary lods, Material material, string name, uint flags)
    {
...
{
    /// <inheritdoc cref="DrawListBegin(Rid, InitialAction, FinalAction, InitialAction, FinalAction, Color[], float, uint, Nullable{Rect2}, Bradot.Collections.Array{Rid})"/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public long DrawListBegin(Rid framebuffer, InitialAction initialColorAction, FinalAction finalColorAction, InitialAction initialDepthAction, FinalAction finalDepthAction, Color[] clearColorValues, float clearDepth, uint clearStencil, Nullable<Rect2> region, Bradot.Collections.Array storageTextures)
    {
        return DrawListBegin(framebuffer, initialColorAction, finalColorAction, initialDepthAction, finalDepthAction, clearColorValues, clearDepth, clearStencil, region, new Bradot.Collections.Array<Rid>(storageTextures));
    }
}
...
{
    /// <inheritdoc cref="AddTriangleFan(Vector3[], Vector2[], Color[], Vector2[], Vector3[], Bradot.Collections.Array{Plane})"/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public void AddTriangleFan(Vector3[] vertices, Vector2[] uvs, Color[] colors, Vector2[] uv2S, Vector3[] normals, Bradot.Collections.Array tangents)
    {
        AddTriangleFan(vertices, uvs, colors, uv2S, normals, new Bradot.Collections.Array<Plane>(tangents));
    }
