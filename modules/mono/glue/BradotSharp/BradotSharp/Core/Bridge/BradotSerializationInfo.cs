...
public sealed class BradotSerializationInfo : IDisposable
{
    ...
    private BradotSerializationInfo(in bradot_dictionary properties, in bradot_dictionary signalEvents)
    {
    ...
    internal static BradotSerializationInfo CreateCopyingBorrowed(
        in bradot_dictionary properties, in bradot_dictionary signalEvents)
    {
        return new(NativeFuncs.bradotsharp_dictionary_new_copy(properties),
            NativeFuncs.bradotsharp_dictionary_new_copy(signalEvents));
    }
        ...
        where T : Delegate
    {
        ...
        {
            if (DelegateUtils.TryDeserializeDelegate(serializedData.AsBradotArray(), out var eventDelegate))
            {
