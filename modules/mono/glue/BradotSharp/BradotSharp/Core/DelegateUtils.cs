...
namespace Bradot
{
    internal static class DelegateUtils
    {
        ...
        internal static bradot_bool DelegateEquals(IntPtr delegateGCHandleA, IntPtr delegateGCHandleB)
        {
            try
            {
                ...
                return (@delegateA! == @delegateB!).ToBradotBool();
            }
            catch (Exception e)
            {
        ...
        [UnmanagedCallersOnly]
        internal static unsafe int GetArgumentCount(IntPtr delegateGCHandle, bradot_bool* outIsValid)
        {
            try
            {
                var @delegate = (Delegate?)GCHandle.FromIntPtr(delegateGCHandle).Target;
                ...
                if (argCount is null)
                {
                    *outIsValid = bradot_bool.False;
                    return 0;
                }
                *outIsValid = bradot_bool.True;
        ...
        internal static unsafe void InvokeWithVariantArgs(IntPtr delegateGCHandle, void* trampoline,
            bradot_variant** args, int argc, bradot_variant* outRet)
        {
            try
            {
                if (trampoline == null)
                ...
                var @delegate = (Delegate)GCHandle.FromIntPtr(delegateGCHandle).Target!;
                var trampolineFn = (delegate* managed<object, NativeVariantPtrArgs, out bradot_variant, void>)trampoline;

                trampolineFn(@delegate, new NativeVariantPtrArgs(args, argc), out bradot_variant ret);
        ...
        private static bool TrySerializeSingleDelegate(Delegate @delegate, [MaybeNullWhen(false)] out byte[] buffer)
        {
            ...
            switch (target)
            {
                ...
                case BradotObject godotObject:
                {
                    using (var stream = new MemoryStream())
                    using (var writer = new BinaryWriter(stream))
                    {
                        writer.Write((ulong)TargetKind.BradotObject);
                    ...
                    if (targetType.IsDefined(typeof(CompilerGeneratedAttribute), true))
                    {
                        ...
                        using (var writer = new BinaryWriter(stream))
                        {
                            ...
                            foreach (FieldInfo field in fields)
                            {
                                ...
                                Variant.Type variantType = BR.TypeToVariantType(fieldType);

                                if (variantType == Variant.Type.Nil)
                                    return false;

                                static byte[] VarToBytes(in bradot_variant var)
                                {
                                    NativeFuncs.bradotsharp_var_to_bytes(var, bradot_bool.True, out var varBytes);
                                    using (varBytes)
        ...
        [UnmanagedCallersOnly]
        internal static unsafe bradot_bool TrySerializeDelegateWithGCHandle(IntPtr delegateGCHandle,
            bradot_array* nSerializedData)
        {
            try
            {
                var serializedData = Collections.Array.CreateTakingOwnershipOfDisposableValue(
                    NativeFuncs.bradotsharp_array_new_copy(*nSerializedData));
        ...
        [UnmanagedCallersOnly]
        internal static unsafe bradot_bool TryDeserializeDelegateWithGCHandle(bradot_array* nSerializedData,
        ...
        private static bool TryDeserializeSingleDelegate(byte[] buffer, [MaybeNullWhen(false)] out Delegate @delegate)
        {
            ...
            using (var reader = new BinaryReader(stream))
            {
                ...
                switch (targetKind)
                {
                    ...
                    case TargetKind.BradotObject:
                    {
                        ulong objectId = reader.ReadUInt64();
                        BradotObject? godotObject = BradotObject.InstanceFromId(objectId);
                        if (godotObject == null)
                            return false;
                    ...
                    case TargetKind.CompilerGenerated:
                    {
                        ...
                        for (int i = 0; i < fieldCount; i++)
                        {
                            ...
                            if (fieldInfo != null)
                            {
                                var variantValue = BR.BytesToVarWithObjects(valueBuffer);
                                object? managedValue = RuntimeTypeConversionHelper.ConvertToObjectOfType(
                                    (bradot_variant)variantValue.NativeVar, fieldInfo.FieldType);
...
#pragma warning restore REFL045
        ...
        private static Type? DeserializeType(BinaryReader reader)
        {
            ...
            {
                BR.PushError($"Missing assembly name of type when attempting to deserialize delegate");
                return null;
            }
        ...
        {
            public static bradot_variant ConvertToVariant(object? obj)
            {
                if (obj == null)
                    return default;
                ...
                switch (obj)
                {
                    ...
                    case BradotObject[] godotObjectArray:
                    ...
                    case Collections.Dictionary bradotDictionary:
                        return VariantUtils.CreateFrom(bradotDictionary);
                    ...
                    case Collections.IGenericBradotDictionary bradotDictionary:
                        return VariantUtils.CreateFrom(bradotDictionary.UnderlyingDictionary);
                    case Collections.IGenericBradotArray godotArray:
                        return VariantUtils.CreateFrom(godotArray.UnderlyingArray);
                }

                BR.PushError("Attempted to convert an unmarshallable managed type to Variant. Name: '" +
                             obj.GetType().FullName + ".");
                return new bradot_variant();
            }

            private delegate object? ConvertToSystemObjectFunc(in bradot_variant managed);
            ...
            private static readonly System.Collections.Generic.Dictionary<Type, ConvertToSystemObjectFunc>
                ...
                {
                    [typeof(bool)] = (in bradot_variant variant) => VariantUtils.ConvertTo<bool>(variant),
                    [typeof(char)] = (in bradot_variant variant) => VariantUtils.ConvertTo<char>(variant),
                    [typeof(sbyte)] = (in bradot_variant variant) => VariantUtils.ConvertTo<sbyte>(variant),
                    [typeof(short)] = (in bradot_variant variant) => VariantUtils.ConvertTo<short>(variant),
                    [typeof(int)] = (in bradot_variant variant) => VariantUtils.ConvertTo<int>(variant),
                    [typeof(long)] = (in bradot_variant variant) => VariantUtils.ConvertTo<long>(variant),
                    [typeof(byte)] = (in bradot_variant variant) => VariantUtils.ConvertTo<byte>(variant),
                    [typeof(ushort)] = (in bradot_variant variant) => VariantUtils.ConvertTo<ushort>(variant),
                    [typeof(uint)] = (in bradot_variant variant) => VariantUtils.ConvertTo<uint>(variant),
                    [typeof(ulong)] = (in bradot_variant variant) => VariantUtils.ConvertTo<ulong>(variant),
                    [typeof(float)] = (in bradot_variant variant) => VariantUtils.ConvertTo<float>(variant),
                    [typeof(double)] = (in bradot_variant variant) => VariantUtils.ConvertTo<double>(variant),
                    [typeof(Vector2)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Vector2>(variant),
                    [typeof(Vector2I)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Vector2I>(variant),
                    [typeof(Rect2)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Rect2>(variant),
                    [typeof(Rect2I)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Rect2I>(variant),
                    [typeof(Transform2D)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Transform2D>(variant),
                    [typeof(Vector3)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Vector3>(variant),
                    [typeof(Vector3I)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Vector3I>(variant),
                    [typeof(Basis)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Basis>(variant),
                    [typeof(Quaternion)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Quaternion>(variant),
                    [typeof(Transform3D)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Transform3D>(variant),
                    [typeof(Vector4)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Vector4>(variant),
                    [typeof(Vector4I)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Vector4I>(variant),
                    [typeof(Aabb)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Aabb>(variant),
                    [typeof(Color)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Color>(variant),
                    [typeof(Plane)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Plane>(variant),
                    [typeof(Callable)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Callable>(variant),
                    [typeof(Signal)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Signal>(variant),
                    [typeof(string)] = (in bradot_variant variant) => VariantUtils.ConvertTo<string>(variant),
                    [typeof(byte[])] = (in bradot_variant variant) => VariantUtils.ConvertTo<byte[]>(variant),
                    [typeof(int[])] = (in bradot_variant variant) => VariantUtils.ConvertTo<int[]>(variant),
                    [typeof(long[])] = (in bradot_variant variant) => VariantUtils.ConvertTo<long[]>(variant),
                    [typeof(float[])] = (in bradot_variant variant) => VariantUtils.ConvertTo<float[]>(variant),
                    ...
                    [typeof(Color[])] = (in bradot_variant variant) => VariantUtils.ConvertTo<Color[]>(variant),
                    [typeof(StringName[])] =
                        (in bradot_variant variant) => VariantUtils.ConvertTo<StringName[]>(variant),
                    [typeof(NodePath[])] = (in bradot_variant variant) => VariantUtils.ConvertTo<NodePath[]>(variant),
                    [typeof(Rid[])] = (in bradot_variant variant) => VariantUtils.ConvertTo<Rid[]>(variant),
                    [typeof(StringName)] = (in bradot_variant variant) => VariantUtils.ConvertTo<StringName>(variant),
                    [typeof(NodePath)] = (in bradot_variant variant) => VariantUtils.ConvertTo<NodePath>(variant),
                    [typeof(Rid)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Rid>(variant),
                    [typeof(Bradot.Collections.Dictionary)] = (in bradot_variant variant) =>
                        VariantUtils.ConvertTo<Bradot.Collections.Dictionary>(variant),
                    [typeof(Bradot.Collections.Array)] =
                        (in bradot_variant variant) => VariantUtils.ConvertTo<Bradot.Collections.Array>(variant),
                    [typeof(Variant)] = (in bradot_variant variant) => VariantUtils.ConvertTo<Variant>(variant),
                };

            public static object? ConvertToObjectOfType(in bradot_variant variant, Type type)
            {
                if (_toSystemObjectFuncByType.TryGetValue(type, out var func))
                    return func(variant);

                if (typeof(BradotObject).IsAssignableFrom(type))
                    return VariantUtils.ConvertTo<BradotObject>(variant);

                if (typeof(BradotObject[]).IsAssignableFrom(type))
                {
                    static BradotObject[] ConvertToSystemArrayOfBradotObject(in bradot_array nativeArray, Type type)
                    {
                        var array = Collections.Array.CreateTakingOwnershipOfDisposableValue(
                            NativeFuncs.bradotsharp_array_new_copy(nativeArray));

                        int length = array.Count;
                        var ret = (BradotObject[])Activator.CreateInstance(type, length)!;

                        for (int i = 0; i < length; i++)
                            ret[i] = array[i].AsBradotObject();

                        return ret;
                    }

                    using var godotArray = NativeFuncs.bradotsharp_variant_as_array(variant);
                    return ConvertToSystemArrayOfBradotObject(godotArray, type);
                }
                ...
                if (type.IsGenericType)
                {
                    ...
                    if (genericTypeDef == typeof(Bradot.Collections.Dictionary<,>))
                    {
                        var ctor = type.GetConstructor(new[] { typeof(Bradot.Collections.Dictionary) });
                    ...
                    if (genericTypeDef == typeof(Bradot.Collections.Array<>))
                    {
                        var ctor = type.GetConstructor(new[] { typeof(Bradot.Collections.Array) });
                        ...
                        return ctor.Invoke(new object?[]
                        {
                            VariantUtils.ConvertTo<Bradot.Collections.Array>(variant)
                        });
                    }
                }

                BR.PushError($"Attempted to convert Variant to unsupported type. Name: {type.FullName}.");
                return null;
            }
        }
    }
