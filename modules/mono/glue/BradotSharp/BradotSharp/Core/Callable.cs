...
namespace Bradot
{
    ...
    {
        private readonly BradotObject _target;
        private readonly StringName _method;
        private readonly Delegate _delegate;
        private readonly unsafe delegate* managed<object, NativeVariantPtrArgs, out bradot_variant, void> _trampoline;

        /// <summary>
        /// Object that contains the method.
        /// </summary>
        public BradotObject Target => _target;
        ...
        /// </summary>
        public unsafe delegate* managed<object, NativeVariantPtrArgs, out bradot_variant, void> Trampoline
            => _trampoline;
        ...
        public unsafe Callable(BradotObject target, StringName method)
        {
            _target = target;
            _method = method;
            _delegate = null;
        ...
        {
            _target = @delegate?.Target as BradotObject;
            _method = null;
            _delegate = @delegate;
        ...
        {
            using bradot_callable callable = Marshaling.ConvertCallableToNative(this);

            int argc = args.Length;

            Span<bradot_variant.movable> argsStoreSpan = argc <= VarArgsSpanThreshold ?
                stackalloc bradot_variant.movable[VarArgsSpanThreshold] :
                new bradot_variant.movable[argc];

            Span<IntPtr> argsSpan = argc <= VarArgsSpanThreshold ?
                stackalloc IntPtr[VarArgsSpanThreshold] :
                new IntPtr[argc];

            fixed (bradot_variant* varargs = &MemoryMarshal.GetReference(argsStoreSpan).DangerousSelfRef)
            fixed (IntPtr* argsPtr = &MemoryMarshal.GetReference(argsSpan))
            {
                for (int i = 0; i < argc; i++)
                {
                    varargs[i] = (bradot_variant)args[i].NativeVar;
                    argsPtr[i] = new IntPtr(&varargs[i]);
                }

                bradot_variant ret = NativeFuncs.bradotsharp_callable_call(callable,
                    (bradot_variant**)argsPtr, argc, out bradot_variant_call_error vcall_error);
                ExceptionUtils.DebugCheckCallError(callable, (bradot_variant**)argsPtr, argc, vcall_error);
        ...
        public unsafe void CallDeferred(params Variant[] args)
        {
            ...
            fixed (IntPtr* argsPtr = &MemoryMarshal.GetReference(argsSpan))
            {
                ...
                NativeFuncs.bradotsharp_callable_call_deferred(callable, (bradot_variant**)argsPtr, argc);
            }
        }
        ...
        ///    <item>
        ///        <term>args</term>
        ///        <description>Array of <see cref="bradot_variant"/> arguments.</description>
        ///    </item>
        ///    <item>
        ///        <term>ret</term>
        ///        <description>Return value of type <see cref="bradot_variant"/>.</description>
        ///    </item>
        ///</list>
        /// <para>
        ...
        ///
        /// <code>
        ///     static void Trampoline(object delegateObj, NativeVariantPtrArgs args, out bradot_variant ret)
        ///     {
        ///         if (args.Count != 1)
