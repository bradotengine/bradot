...
namespace BradotTools.Build
{
    public static class BuildSystem
    {
        ...
        {
            var verbosityLevel =
                editorSettings.GetSetting(BradotSharpEditor.Settings.VerbosityLevel).As<VerbosityLevelId>();
            arguments.Add("-v");
            ...
            if ((bool)editorSettings.GetSetting(BradotSharpEditor.Settings.NoConsoleLogging))
                arguments.Add("-noconlog");
        }

        private static void AddLoggerArgument(BuildInfo buildInfo, Collection<string> arguments)
        {
            string buildLoggerPath = Path.Combine(Internals.BradotSharpDirs.DataEditorToolsDir,
                "BradotTools.BuildLogger.dll");

            arguments.Add(
                $"-l:{typeof(BradotBuildLogger).FullName},{buildLoggerPath};{buildInfo.LogsDirPath}");
        }

        private static void AddBinaryLogArgument(BuildInfo buildInfo, Collection<string> arguments,
            EditorSettings editorSettings)
        {
            if (!(bool)editorSettings.GetSetting(BradotSharpEditor.Settings.CreateBinaryLog))
                return;
        ...
        {
            var baseDylib = $"{BradotSharpDirs.ProjectAssemblyName}.dylib";
            var baseSym = $"{BradotSharpDirs.ProjectAssemblyName}.framework.dSYM";
