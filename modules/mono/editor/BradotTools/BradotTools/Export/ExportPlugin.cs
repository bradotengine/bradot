...
namespace BradotTools.Export
{
    public partial class ExportPlugin : EditorExportPlugin
    {
        ...
        {
            return File.Exists(BradotSharpDirs.ProjectSlnPath);
        }
        ...
        public override Bradot.Collections.Array<Bradot.Collections.Dictionary> _GetExportOptions(EditorExportPlatform platform)
        {
            return new Bradot.Collections.Array<Bradot.Collections.Dictionary>()
            {
                new Bradot.Collections.Dictionary()
                {
                    {
                        "option", new Bradot.Collections.Dictionary()
                        {
        ...
        public override void _ExportFile(string path, string type, string[] features)
        {
            ...
            if (!ProjectContainsDotNet())
            {
                GetExportPlatform().AddMessage(EditorExportPlatform.ExportMessageType.Error, "Export .NET Project", $"This project contains C# files but no solution file was found at the following path: {BradotSharpDirs.ProjectSlnPath}\n" +
            ...
            if (!includeScriptsContent)
            {
                ...
                // Sadly, Bradot prints errors when adding an empty file (nothing goes wrong, it's just noise).
                // Because of this, we add a file which contains a line break.
                AddFile(path, System.Text.Encoding.UTF8.GetBytes("\n"), remap: false);

                // Tell the Bradot exporter that we already took care of the file.
                Skip();
            }
        }
        ...
        private void _ExportBeginImpl(string[] features, bool isDebug, string path, long flags)
        {
            ...
            foreach (PublishConfig config in targets)
            {
                ...
                foreach (string arch in config.Archs)
                {
                    ...
                    string projectDataDirName = $"data_{BradotSharpDirs.CSharpProjectName}_{platform}_{arch}";
                    if (platform == OS.Platforms.MacOS)
                    {
                    ...
                    {
                        publishOutputDir = Path.Combine(BradotSharpDirs.ProjectBaseOutputPath, "godot-publish-dotnet",
                    ...
                    string assemblyPath = Path.Combine(publishOutputDir, $"{BradotSharpDirs.ProjectAssemblyName}.dll");
                    string nativeAotPath = Path.Combine(publishOutputDir,
                        $"{BradotSharpDirs.ProjectAssemblyName}.{soExt}");
                    ...
                    RecursePublishContents(publishOutputDir,
                        ...
                        filterFile: file =>
                        {
                            if (platform == OS.Platforms.iOS)
                            {
                                ...
                                return Path.GetFileName(file) != $"{BradotSharpDirs.ProjectAssemblyName}.dylib";
                            }
                        ...
                        addEntry: (path, isFile) =>
                        {
                            ...
                            if (isFile)
                            {
                                if (embedBuildResults)
                                {
                                    ...
                                    AddFile($"res://.bradot/mono/publish/{arch}/{filePath}", fileData, false);
                                }
                                else
                                {
                    ...
                    if (embedBuildResults)
                    {
                        ...
                        AddFile($"res://.bradot/mono/publish/{arch}/.dotnet-publish-manifest", fileData, false);
                    }
                }
            }
            ...
            if (platform == OS.Platforms.iOS)
            {
                if (outputPaths.Count > 2)
                {
                    ...
                    string outputPath = Path.Combine(outputPaths[1], $"{BradotSharpDirs.ProjectAssemblyName}.dylib");
                    string[] files = outputPaths
                        .Skip(1)
                        .Select(path => Path.Combine(path, $"{BradotSharpDirs.ProjectAssemblyName}.dylib"))
                        .ToArray();
                ...
                string xcFrameworkPath = Path.Combine(BradotSharpDirs.ProjectBaseOutputPath, publishConfig.BuildConfig, $"{BradotSharpDirs.ProjectAssemblyName}_aot.xcframework");
