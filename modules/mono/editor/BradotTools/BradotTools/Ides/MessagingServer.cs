...
namespace BradotTools.Ides
{
    public sealed class MessagingServer : IDisposable
    {
        ...
        public MessagingServer(string editorExecutablePath, string projectMetadataDir, ILogger logger)
        {
            ...
            _metaFilePath = Path.Combine(projectMetadataDir, BradotIdeMetadata.DefaultFileName);

            // Make sure the directory exists
            Directory.CreateDirectory(projectMetadataDir);

            // The Bradot editor's file system thread can keep the file open for writing, so we are forced to allow write sharing...
            ...
            where TResponse : Response, new()
        {
            using (await _peersSem.UseAsync())
            {
                ...
                {
                    _logger.LogError("Cannot write request. No client connected to the Bradot Ide Server.");
                    return;
                }
        ...
        private class ServerMessageHandler : IMessageHandler
        {
            private static void DispatchToMainThread(Action action)
            {
                ...
                Bradot.Dispatcher.SynchronizationContext.Post(d, null);
            }
            ...
            private static Task<Response> HandleDebugPlay(DebugPlayRequest request)
            {
                DispatchToMainThread(() =>
                {
                    ...
                    BradotSharpEditor.Instance.SkipBuildBeforePlaying = !(request.BuildBeforePlaying ?? true);
                    ...
                    Environment.SetEnvironmentVariable("BRADOT_MONO_DEBUGGER_AGENT",
                    ...
                    Environment.SetEnvironmentVariable("BRADOT_MONO_DEBUGGER_AGENT", "");
                    BradotSharpEditor.Instance.SkipBuildBeforePlaying = false;
                });
            ...
            private static async Task<Response> HandleCodeCompletionRequest(CodeCompletionRequest request)
            {
                ...
                await Bradot.Engine.GetMainLoop().ToSignal(Bradot.Engine.GetMainLoop(), "process_frame");
