...
namespace BradotTools.Ides.Rider
{
    public static class RiderPathManager
    {
        ...
        {
            var editorSettings = EditorInterface.Singleton.GetEditorSettings();
            var editor = editorSettings.GetSetting(BradotSharpEditor.Settings.ExternalEditor).As<ExternalEditorId>();
            if (editor == ExternalEditorId.Rider)
            {
                if (!editorSettings.HasSetting(EditorPathSettingName))
                {
                    Globals.EditorDef(EditorPathSettingName, "");
                    editorSettings.AddPropertyInfo(new Bradot.Collections.Dictionary
                    {
        ...
        public static void OpenFile(string slnPath, string scriptPath, int line, int column)
        {
            ...
            {
                BR.PushError($"Error when trying to run code editor: JetBrains Rider or Fleet. Could not find path to the editor.");
                return;
            }
