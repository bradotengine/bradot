...
namespace BradotTools.Utils
{
    ...
    public static class OS
    {
        ...
        public static readonly Dictionary<string, string> DotNetOSPlatformMap = new Dictionary<string, string>
        {
            ...
            // TODO: Does .NET 6 support BSD variants? If it does, it may need the name `unix`
            // instead of `linux` in the runtime identifier. This would be a problem as
            // Bradot has a single export profile for both, named LinuxBSD.
        ...
        {
            Internal.bradot_icall_Utils_OS_GetPlatformName(out bradot_string dest);
            using (dest)
            {
        ...
        private static string? PathWhichUnix(string name)
        {
            ...
            return searchDirs.Select(dir => Path.Combine(dir, name))
                .FirstOrDefault(path =>
                {
                    using bradot_string pathIn = Marshaling.ConvertStringToNative(path);
                    return File.Exists(path) && Internal.bradot_icall_Utils_OS_UnixFileHasExecutableAccess(pathIn);
                });
        }
