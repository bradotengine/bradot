...
using BradotTools.IdeMessaging.Utils;
...
namespace BradotTools.IdeMessaging
{
    ...
    public sealed class Client : IDisposable
    {
        ...
        private BradotIdeMetadata bradotIdeMetadata;
        private readonly FileSystemWatcher fsWatcher;

        public string BradotEditorExecutablePath => bradotIdeMetadata.EditorExecutablePath;
        ...
        public Client(string identity, string godotProjectDir, IMessageHandler messageHandler, ILogger logger)
        {
            ...
            string projectMetadataDir = Path.Combine(godotProjectDir, ".bradot", "mono", "metadata");
            // FileSystemWatcher requires an existing directory
            if (!Directory.Exists(projectMetadataDir))
            {
                // Check if the non hidden version exists
                string nonHiddenProjectMetadataDir = Path.Combine(godotProjectDir, "bradot", "mono", "metadata");
            ...
            MetaFilePath = Path.Combine(projectMetadataDir, BradotIdeMetadata.DefaultFileName);

            fsWatcher = new FileSystemWatcher(projectMetadataDir, BradotIdeMetadata.DefaultFileName);
        }
        ...
        private async void OnMetaFileChanged(object sender, FileSystemEventArgs e)
        {
            ...
            using (await connectionSem.UseAsync())
            {
                ...
                if (metadata != null && metadata != bradotIdeMetadata)
                {
                    bradotIdeMetadata = metadata.Value;
        ...
        private BradotIdeMetadata? ReadMetadataFile()
        {
            ...
            using (var reader = new StreamReader(fileStream))
            {
                ...
                return new BradotIdeMetadata(port, editorExecutablePath);
            }
        }
        ...
        private async Task ConnectToServer()
        {
            ...
            try
            {
                ...
                await tcpClient.ConnectAsync(IPAddress.Loopback, bradotIdeMetadata.Port);
