...
namespace Bradot.SourceGenerators.Sample
{
    ...
    public partial class ExportedProperties : BradotObject
    {
        ...
        [Export]
        private Bradot.Collections.Dictionary PropertyBradotDictionary { get; set; } =
            new() { { "foo", 10 }, { Vector2.Up, Colors.Chocolate } };

        [Export]
        private Bradot.Collections.Array PropertyBradotArray { get; set; } =
            new() { "foo", 10, Vector2.Up, Colors.Chocolate };

        [Export]
        private Bradot.Collections.Dictionary<string, bool> PropertyBradotGenericDictionary { get; set; } =
            new() { { "foo", true }, { "bar", false } };

        [Export]
        private Bradot.Collections.Array<int> PropertyBradotGenericArray { get; set; } =
            new() { 0, 1, 2, 3, 4, 5, 6 };
    }
}
