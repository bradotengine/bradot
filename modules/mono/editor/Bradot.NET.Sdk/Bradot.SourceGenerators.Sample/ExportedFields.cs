...
namespace Bradot.SourceGenerators.Sample
{
    ...
    public partial class ExportedFields : BradotObject
    {
        ...
        [Export]
        private Bradot.Collections.Dictionary _fieldBradotDictionary =
            new() { { "foo", 10 }, { Vector2.Up, Colors.Chocolate } };

        [Export]
        private Bradot.Collections.Array _fieldBradotArray =
            new() { "foo", 10, Vector2.Up, Colors.Chocolate };

        [Export]
        private Bradot.Collections.Dictionary<string, bool> _fieldBradotGenericDictionary =
            new() { { "foo", true }, { "bar", false } };

        [Export]
        private Bradot.Collections.Array<int> _fieldBradotGenericArray =
            new() { 0, 1, 2, 3, 4, 5, 6 };
    }
}
