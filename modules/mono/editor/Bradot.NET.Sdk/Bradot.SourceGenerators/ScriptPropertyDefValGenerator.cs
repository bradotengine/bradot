...
namespace Bradot.SourceGenerators
{
    ...
    public class ScriptPropertyDefValGenerator : ISourceGenerator
    {
        ...
        {
            if (context.IsBradotSourceGeneratorDisabled("ScriptPropertyDefVal"))
                return;
        ...
        )
        {
            ...
            bool isNode = symbol.InheritsFrom("BradotSharp", BradotClasses.Node);
            ...
            var exportedProperties = members
                ...
                .Where(s => s.GetAttributes()
                    .Any(a => a.AttributeClass?.IsBradotExportAttribute() ?? false))
                .ToArray();
            ...
            foreach (var property in exportedProperties)
            {
                ...
                // Two possible ways: reflection or turn RestoreBradotObjectData into a constructor overload.
                // Ignore properties without a getter, without a setter or with an init-only setter.
                // Bradot properties must be both readable and writable.
            ...
            foreach (var field in exportedFields)
            {
                ...
                // Ignore properties without a getter or without a setter. Bradot properties must be both readable and writable.
                if (field.IsReadOnly)
                {
                    context.ReportDiagnostic(Diagnostic.Create(
                        Common.ExportedMemberIsReadOnlyRule,
            ...
            // Generate GetBradotExportedProperties
            ...
            if (exportedMembers.Count > 0)
            {
                ...
                const string DictionaryType =
                    "global::System.Collections.Generic.Dictionary<global::Bradot.StringName, global::Bradot.Variant>";

                source.Append("#if TOOLS\n");

                source.Append("    /// <summary>\n")
                    .Append("    /// Get the default values for all properties declared in this class.\n")
                    .Append("    /// This method is used by Bradot to determine the value that will be\n")
                ...
                source.Append(" GetBradotPropertyDefaultValues()\n    {\n");
        ...
        {
            if (marshalType == MarshalType.BradotObjectOrDerived)
            {
                return memberType.InheritsFrom("BradotSharp", BradotClasses.Node);
            }
            if (marshalType == MarshalType.BradotObjectOrDerivedArray)
            {
                var elementType = ((IArrayTypeSymbol)memberType).ElementType;
                return elementType.InheritsFrom("BradotSharp", BradotClasses.Node);
            }
            if (memberType is INamedTypeSymbol { IsGenericType: true } genericType)
            {
                return genericType.TypeArguments
                    .Any(static typeArgument
                        => typeArgument.InheritsFrom("BradotSharp", BradotClasses.Node));
            }
