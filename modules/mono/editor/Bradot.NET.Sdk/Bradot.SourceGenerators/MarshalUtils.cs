...
namespace Bradot.SourceGenerators
{
    internal static class MarshalUtils
    {
        ...
        {
            public INamedTypeSymbol BradotObjectType { get; }
            ...
            public TypeCache(Compilation compilation)
            {
                ...
                BradotObjectType = GetTypeByMetadataNameOrThrow(BradotClasses.BradotObject);
            }
        }
        ...
        public static VariantType? ConvertMarshalTypeToVariantType(MarshalType marshalType)
            => marshalType switch
            {
                ...
                MarshalType.BradotObjectOrDerivedArray => VariantType.Array,
                ...
                MarshalType.BradotObjectOrDerived => VariantType.Object,
                MarshalType.StringName => VariantType.StringName,
                MarshalType.NodePath => VariantType.NodePath,
                MarshalType.Rid => VariantType.Rid,
                MarshalType.BradotDictionary => VariantType.Dictionary,
                MarshalType.BradotArray => VariantType.Array,
                MarshalType.BradotGenericDictionary => VariantType.Dictionary,
                MarshalType.BradotGenericArray => VariantType.Array,
                _ => null
            };
        ...
        public static MarshalType? ConvertManagedTypeToMarshalType(ITypeSymbol type, TypeCache typeCache)
        {
            ...
            switch (specialType)
            {
                ...
                default:
                {
                    ...
                    {
                        if (type.ContainingAssembly?.Name == "BradotSharp" &&
                            type.ContainingNamespace?.Name == "Bradot")
                        {
                            return type switch
                            {
                    ...
                    else if (typeKind == TypeKind.Array)
                    {
                        ...
                        if (elementType.SimpleDerivesFrom(typeCache.BradotObjectType))
                            return MarshalType.BradotObjectOrDerivedArray;

                        if (elementType.ContainingAssembly?.Name == "BradotSharp" &&
                            elementType.ContainingNamespace?.Name == "Bradot")
                        {
                            switch (elementType)
                            {
                                case { Name: "Vector2" }:
                    ...
                    {
                        if (type.SimpleDerivesFrom(typeCache.BradotObjectType))
                            return MarshalType.BradotObjectOrDerived;
                        ...
                        if (type.ContainingAssembly?.Name == "BradotSharp")
                        {
                            switch (type.ContainingNamespace?.Name)
                            {
                                ...
                                case "Collections"
                                    when type.ContainingNamespace?.FullQualifiedNameOmitGlobal() == "Bradot.Collections":
                                    return type switch
                                    {
                                        { Name: "Dictionary" } =>
                                            type is INamedTypeSymbol { IsGenericType: false } ?
                                                MarshalType.BradotDictionary :
                                                MarshalType.BradotGenericDictionary,
                                        { Name: "Array" } =>
                                            type is INamedTypeSymbol { IsGenericType: false } ?
                                                MarshalType.BradotArray :
                                                MarshalType.BradotGenericArray,
                                        _ => null
                                    };
                            }
                        }
        ...
        private const string VariantUtils = "global::Bradot.NativeInterop.VariantUtils";
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            ...
            {
                // We need a special case for BradotObjectOrDerived[], because it's not supported by VariantUtils.ConvertTo<T>
                MarshalType.BradotObjectOrDerivedArray =>
                    source.Append(VariantUtils, ".ConvertToSystemArrayOfBradotObject<",
                        ((IArrayTypeSymbol)typeSymbol).ElementType.FullQualifiedNameIncludeGlobal(), ">(",
                        inputExpr, ")"),
                // We need a special case for generic Bradot collections and BradotObjectOrDerived[], because VariantUtils.ConvertTo<T> is slower
                MarshalType.BradotGenericDictionary =>
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            return marshalType switch
            {
                // We need a special case for BradotObjectOrDerived[], because it's not supported by VariantUtils.CreateFrom<T>
                MarshalType.BradotObjectOrDerivedArray =>
                    source.Append(VariantUtils, ".CreateFromSystemArrayOfBradotObject(", inputExpr, ")"),
                // We need a special case for generic Bradot collections and BradotObjectOrDerived[], because VariantUtils.CreateFrom<T> is slower
                MarshalType.BradotGenericDictionary =>
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            return marshalType switch
            {
                // We need a special case for BradotObjectOrDerived[], because it's not supported by Variant.As<T>
                MarshalType.BradotObjectOrDerivedArray =>
                    source.Append(inputExpr, ".AsBradotObjectArray<",
                        ((IArrayTypeSymbol)typeSymbol).ElementType.FullQualifiedNameIncludeGlobal(), ">()"),
                // We need a special case for generic Bradot collections and BradotObjectOrDerived[], because Variant.As<T> is slower
                MarshalType.BradotGenericDictionary =>
                    source.Append(inputExpr, ".AsBradotDictionary<",
                        ((INamedTypeSymbol)typeSymbol).TypeArguments[0].FullQualifiedNameIncludeGlobal(), ", ",
                        ((INamedTypeSymbol)typeSymbol).TypeArguments[1].FullQualifiedNameIncludeGlobal(), ">()"),
                MarshalType.BradotGenericArray =>
                    source.Append(inputExpr, ".AsBradotArray<",
                        ((INamedTypeSymbol)typeSymbol).TypeArguments[0].FullQualifiedNameIncludeGlobal(), ">()"),
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            return marshalType switch
            {
                // We need a special case for BradotObjectOrDerived[], because it's not supported by Variant.From<T>
                MarshalType.BradotObjectOrDerivedArray =>
                    source.Append("global::Bradot.Variant.CreateFrom(", inputExpr, ")"),
                // We need a special case for generic Bradot collections, because Variant.From<T> is slower
                MarshalType.BradotGenericDictionary or MarshalType.BradotGenericArray =>
                    source.Append("global::Bradot.Variant.CreateFrom(", inputExpr, ")"),
                _ => source.Append("global::Bradot.Variant.From<",
                    typeSymbol.FullQualifiedNameIncludeGlobal(), ">(", inputExpr, ")")
            };
        }
    }
