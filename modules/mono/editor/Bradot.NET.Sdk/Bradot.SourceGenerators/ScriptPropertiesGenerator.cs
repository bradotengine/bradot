...
namespace Bradot.SourceGenerators
{
    ...
    public class ScriptPropertiesGenerator : ISourceGenerator
    {
        ...
        {
            if (context.IsBradotSourceGeneratorDisabled("ScriptProperties"))
                return;
        ...
        )
        {
            ...
            var bradotClassProperties = propertySymbols.WhereIsBradotCompatibleType(typeCache).ToArray();
            var bradotClassFields = fieldSymbols.WhereIsBradotCompatibleType(typeCache).ToArray();
            ...
            if (bradotClassProperties.Length > 0 || bradotClassFields.Length > 0)
            {

                // Generate SetBradotClassPropertyValue

                bool allPropertiesAreReadOnly = bradotClassFields.All(fi => fi.FieldSymbol.IsReadOnly) &&
                                                bradotClassProperties.All(pi => pi.PropertySymbol.IsReadOnly || pi.PropertySymbol.SetMethod!.IsInitOnly);

                if (!allPropertiesAreReadOnly)
                {
                    source.Append("    /// <inheritdoc/>\n");
                    source.Append("    [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]\n");
                    source.Append("    protected override bool SetBradotClassPropertyValue(in bradot_string_name name, ");
                    source.Append("in bradot_variant value)\n    {\n");
                    ...
                    source.Append("        return base.SetBradotClassPropertyValue(name, value);\n");

                    source.Append("    }\n");
                }

                // Generate GetBradotClassPropertyValue
                bool allPropertiesAreWriteOnly = bradotClassFields.Length == 0 && bradotClassProperties.All(pi => pi.PropertySymbol.IsWriteOnly);

                if (!allPropertiesAreWriteOnly)
                {
                    source.Append("    /// <inheritdoc/>\n");
                    source.Append("    [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]\n");
                    source.Append("    protected override bool GetBradotClassPropertyValue(in bradot_string_name name, ");
                    source.Append("out bradot_variant value)\n    {\n");
                    ...
                    source.Append("        return base.GetBradotClassPropertyValue(name, out value);\n");

                    source.Append("    }\n");
                }
                // Generate GetBradotPropertyList

                const string DictionaryType = "global::System.Collections.Generic.List<global::Bradot.Bridge.PropertyInfo>";

                source.Append("    /// <summary>\n")
                    .Append("    /// Get the property information for all the properties declared in this class.\n")
                    .Append("    /// This method is used by Bradot to register the available properties in the editor.\n")
                ...
                var bradotClassPropertiesAndFields = Enumerable.Empty<BradotPropertyOrFieldData>()
                    .Concat(bradotClassProperties.Select(propertyData => new BradotPropertyOrFieldData(propertyData)))
                    .Concat(bradotClassFields.Select(fieldData => new BradotPropertyOrFieldData(fieldData)))
                    .OrderBy(data => data.Symbol.Locations[0].Path())
                    .ThenBy(data => data.Symbol.Locations[0].StartLine());

                foreach (var member in bradotClassPropertiesAndFields)
                {
        ...
        {
            source.Append("        properties.Add(new(type: (global::Bradot.Variant.Type)")
        ...
        private static IEnumerable<PropertyInfo> DetermineGroupingPropertyInfo(ISymbol memberSymbol)
        {
            foreach (var attr in memberSymbol.GetAttributes())
            {
                ...
                {
                    BradotClasses.ExportCategoryAttr => PropertyUsageFlags.Category,
                    BradotClasses.ExportGroupAttr => PropertyUsageFlags.Group,
                    BradotClasses.ExportSubgroupAttr => PropertyUsageFlags.Subgroup,
                    _ => null
                };
        ...
        )
        {
            var exportAttr = memberSymbol.GetAttributes()
                .FirstOrDefault(a => a.AttributeClass?.IsBradotExportAttribute() ?? false);
        ...
        )
        {
            ...
            if (variantType == VariantType.Object && type is INamedTypeSymbol memberNamedType)
            {
                ...
                if (memberNamedType.InheritsFrom("BradotSharp", "Bradot.Resource"))
                {
                ...
                if (memberNamedType.InheritsFrom("BradotSharp", "Bradot.Node"))
                {
            ...
            static string GetTypeName(INamedTypeSymbol memberSymbol)
            {
                if (memberSymbol.GetAttributes()
                    .Any(a => a.AttributeClass?.IsBradotGlobalClassAttribute() ?? false))
                {
                    return memberSymbol.Name;
                }

                return memberSymbol.GetBradotScriptNativeClassName()!;
            }
