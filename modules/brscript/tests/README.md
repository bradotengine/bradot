# BRScript integration tests

The `scripts/` folder contains integration tests in the form of BRScript files
and output files.

See the
[Integration tests for BRScript documentation](https://docs.bradotengine.org/en/latest/contributing/development/core_and_modules/unit_testing.html#integration-tests-for-brscript)
for information about creating and running BRScript integration tests.

# BRScript Autocompletion tests

The `script/completion` folder contains test for the BRScript autocompletion.

Each test case consists of at least one `.br` file, which contains the code, and one `.cfg` file, which contains expected results and configuration. Inside of the BRScript file the character `➡` represents the cursor position, at which autocompletion is invoked.

The script files won't be parsable BRScript since it contains an invalid char and and often the code is not complete during autocompletion. To allow for a valid base when used with a scene, the
...
- `scene: String`: Allows to specify a scene which is opened while autocompletion is performed. If this is not set the test runner will search for a `.tscn` file with the same basename as the BRScript file. If that isn't found either, autocompletion will behave as if no scene was opened.
...
- All possible types: Test with all possible types that apply to the tested behavior. (For the last points testing against `SCRIPT` and `CLASS` should suffice. `CLASS` can be obtained through C#, `SCRIPT` through BRScript. Relying on autoloads to be of type `SCRIPT` is not good, since this might change in the future.)

  - `BUILTIN`
  - `NATIVE`
  - BRScripts (with `class_name` as well as `preload`ed)
