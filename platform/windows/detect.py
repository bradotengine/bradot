...
def can_build():
    if os.name == "nt":
        ...
        # If VCINSTALLDIR is set in the OS environ, use traditional Bradot logic to set up MSVC
...
def get_opts():
    ...
    if d3d12_deps_folder:
        d3d12_deps_folder = os.path.join(d3d12_deps_folder, "Bradot", "build_deps")
    else:
...
def setup_msvc_auto(env: "SConsEnvironment"):
    ...
    # aliases or aren't supported by Bradot (itanium & ia64).
...
def configure_msvc(env: "SConsEnvironment", vcvars_msvc_config):
    ...
    env.Append(LINKFLAGS=["/NATVIS:platform\\windows\\bradot.natvis"])
...
def check_d3d12_installed(env):
    if not os.path.exists(env["mesa_libs"]):
        print_error(
            ...
            "https://docs.bradotengine.org/en/latest/contributing/development/compiling/compiling_for_windows.html"
        )
        sys.exit(255)
