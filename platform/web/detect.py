...
def get_opts():
    ...
    return [
        ...
        # Matches default values from before Emscripten 3.1.27. New defaults are too low for Bradot.
        ...
        BoolVariable(
            "dlink_enabled", "Enable WebAssembly dynamic linking (BRExtension support). Produces bigger binaries", False
        ),
...
def configure(env: "SConsEnvironment"):
    ...
    if cc_semver < (3, 1, 62):
        print_error("The minimum emscripten version to build Bradot is 3.1.62, detected: %s.%s.%s" % cc_semver)
        sys.exit(255)
    ...
    if env["dlink_enabled"]:
        if env["proxy_to_pthread"]:
            print_warning("BRExtension support requires proxy_to_pthread=no, disabling proxy to pthread.")
            env["proxy_to_pthread"] = False
    ...
    # Wrap the JavaScript support code around a closure named Bradot.
    env.Append(LINKFLAGS=["-sMODULARIZE=1", "-sEXPORT_NAME='Bradot'"])
