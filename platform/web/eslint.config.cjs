...
if (process && process.env && process.env.npm_command && !fs.existsSync('./platform/web/eslint.config.cjs')) {
	throw Error('eslint must be run from the Bradot project root folder');
}
...
module.exports = [
	...
	// javascript templates (service workers)
	{
		...
		languageOptions: {
			globals: {
				...globals.browser,
				'___BRADOT_CACHE___': true,
				'___BRADOT_ENSURE_CROSSORIGIN_ISOLATION_HEADERS___': true,
				'___BRADOT_OPT_CACHE___': true,
			},
		},
	},
	...
	// html templates
	{
		...
		languageOptions: {
			...
			globals: {
				...
				'Engine': true,
				'$BRADOT_CONFIG': true,
				'$BRADOT_PROJECT_NAME': true,
				'$BRADOT_THREADS_ENABLED': true,
				'___BRADOT_THREADS_ENABLED___': true,
			},
		},
		rules: {
