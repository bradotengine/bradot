...
def create_engine_file(env, target, source, externs, threads_enabled):
    ...
    subst_dict = {"___BRADOT_THREADS_ENABLED": "true" if threads_enabled else "false"}
...
def create_template_zip(env, js, wasm, worker, side):
    binary_name = "bradot.editor" if env.editor_build else "bradot"
    ...
    if env["dlink_enabled"]:
        in_files.append(side)  # Side wasm (contains the actual Bradot code).
    ...
    if env.editor_build:
        ...
        cache = [
            ...
            "offline.html",
            "bradot.editor.js",
            "bradot.editor.audio.worklet.js",
            "bradot.editor.audio.position.worklet.js",
            "logo.svg",
            "favicon.png",
        ]
        if env["threads"]:
            cache.append("bradot.editor.worker.js")
        opt_cache = ["bradot.editor.wasm"]
        subst_dict = {
            "___BRADOT_VERSION___": get_build_version(False),
            "___BRADOT_NAME___": "BradotEngine",
            "___BRADOT_CACHE___": json.dumps(cache),
            "___BRADOT_OPT_CACHE___": json.dumps(opt_cache),
            "___BRADOT_OFFLINE_PAGE___": "offline.html",
            "___BRADOT_THREADS_ENABLED___": "true" if env["threads"] else "false",
        }
        html = env.Substfile(target="#bin/bradot${PROGSUFFIX}.html", source=html, SUBST_DICT=subst_dict)
        ...
        service_worker = env.Substfile(
            target="#bin/bradot${PROGSUFFIX}.service.worker.js",
        ...
        out_files.append(zip_dir.File("bradot.offline.html"))
