		...
		const proto = /** @lends Engine.prototype */ {
				...
				function doInit(promise) {
					...
					return new Promise(function (resolve, reject) {
						promise.then(function (response) {
							...
							Bradot(me.config.getModuleConfig(loadPath, cloned)).then(function (module) {
			...
			start: function (override) {
				...
				return me.init().then(function () {
					...
					let config = {};
					try {
						config = me.config.getBradotConfig(function () {
							me.rtenv = null;
						});
					} catch (e) {
					...
					// Preload BRExtension libraries.
					if (me.config.brextensionLibs.length > 0 && !me.rtenv['loadDynamicLibrary']) {
						return Promise.reject(new Error('BRExtension libraries are not supported by this engine version. '
