...
def configure(env: "SConsEnvironment"):
    ...
    if not has_swappy:
        print_warning(
            ...
            + "Without Swappy, Bradot apps on Android will inevitable suffer stutter and struggle to keep consistent 30/60/90/120 fps. Though Swappy cannot guarantee your app will be stutter-free, not having Swappy will guarantee there will be stutter even on the best phones and the most simple of scenes."
        )
        if env["swappy"]:
    ...
    if env["arch"] == "x86_32":
        # The NDK adds this if targeting API < 24, so we can drop it when Bradot targets it at least
    ...
    env.Append(LINKFLAGS="-Wl,-soname,libbradot_android.so")
