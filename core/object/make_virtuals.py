proto = """#define BRVIRTUAL$VER($RET m_name $ARG)\\
	...
	_FORCE_INLINE_ bool _gdvirtual_##m_name##_call($CALLARGS) $CONST {\\
		...
		if (unlikely(_get_extension() && !_gdvirtual_##m_name##_initialized)) {\\
			...
			BRVIRTUAL_TRACK(_gdvirtual_##m_name, _gdvirtual_##m_name##_initialized);\\
		...
		if (_gdvirtual_##m_name) {\\
			...
			if (_get_extension()->get_virtual_call_data && _get_extension()->call_virtual_with_data) {\\
				...
				((BRExtensionClassCallVirtual)_gdvirtual_##m_name)(_get_extension_instance(), $CALLPTRARGPASS, $CALLPTRRETPASS);\\
				$CALLPTRRET\\
			}\\
			return true;\\
		}\\
...
def generate_version(argcount, const=False, returns=False, required=False):
    ...
    if argcount > 0:
        ...
        callptrargsptr = f"\t\t\tBRExtensionConstTypePtr argptrs[{argcount}] = {{ "
    callptrargs = ""
    ...
    if argcount:
        ...
        s = s.replace("$CALLPTRARGPASS", "reinterpret_cast<BRExtensionConstTypePtr *>(argptrs)")
    else:
...
#ifndef BRVIRTUAL_GEN_H
#define BRVIRTUAL_GEN_H

#include "core/object/script_instance.h"

#ifdef TOOLS_ENABLED
#define BRVIRTUAL_TRACK(m_virtual, m_initialized)\\
