...
def generate_scu_files(max_includes_per_scu):
    ...
    if folder_not_found("core") or folder_not_found("platform") or folder_not_found("scene"):
        raise RuntimeError("scu_builders.py must be run from the bradot folder.")
        return
    ...
    process_folder(["modules/bradot_physics_2d"])
    process_folder(["modules/bradot_physics_3d"])
    process_folder(["modules/bradot_physics_3d/joints"])

    process_folder(["modules/csg"])
    process_folder(["modules/brscript"])
    process_folder(["modules/brscript/editor"])
    process_folder(["modules/brscript/language_server"])
