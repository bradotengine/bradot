...
class ClassStatus:
    ...
    def make_output(self) -> Dict[str, str]:
        ...
        if self.name.startswith("Total"):
            output["url"] = color("url", "https://docs.bradotengine.org/en/latest/classes/")
            if flags["s"]:
                output["comment"] = color("part_good", "ALL OK")
        else:
            output["url"] = color(
                "url", "https://docs.bradotengine.org/en/latest/classes/class_{name}.html".format(name=self.name.lower())
            )
