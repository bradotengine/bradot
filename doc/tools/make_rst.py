...
BRADOT_DOCS_PATTERN = re.compile(r"^\$DOCS_URL/(.*)\.html(#.*)?$")
...
# Checks if code samples have both BRScript and C# variations.
# For simplicity we assume that a BRScript example is always present, and ignore contexts
...
class ScriptLanguageParityCheck:
    ...
    def add_hit(self, class_name: str, context: DefinitionBase, error: str, state: State) -> None:
        if class_name in ["@BRScript", "@GlobalScope"]:
...
def make_rst_class(class_def: ClassDef, state: State, dry_run: bool, output_dir: str) -> None:
    ...
    with open(
        ...
        source_github_url = f"https://github.com/bradotengine/bradot/tree/{git_branch}/{source_xml_path}"
        generator_github_url = f"https://github.com/bradotengine/bradot/tree/{git_branch}/doc/tools/make_rst.py"

        f.write(".. DO NOT EDIT THIS FILE!!!\n")
        f.write(".. Generated automatically from Bradot engine sources.\n")
...
def make_link(url: str, title: str) -> str:
    match = BRADOT_DOCS_PATTERN.search(url)
    if match:
        groups = match.groups()
...
RESERVED_CODEBLOCK_TAGS = ["codeblock", "brscript", "csharp"]
...
def format_text_block(
    ...
    while True:
        ...
        if (
            ...
            or post_text.startswith("[brscript]")
            or post_text.startswith("[brscript ")
        ...
        # Tag is a cross-reference or a formatting directive.
            ...
            # Entering codeblocks and inline code tags.
                ...
                if tag_state.closing:
                    if not has_codeblocks_brscript or not has_codeblocks_csharp:
                ...
                if tag_state.name == "brscript":
                    if not inside_code_tabs:
                        print_error(
                            f"{state.current_class}.xml: BRScript code block is used outside of [codeblocks] in {context_name}.",
                            state,
                        )
                    else:
                        has_codeblocks_brscript = True
                    tag_text = "\n .. code-tab:: brscript\n"
